...
- Model connections go through the Yggdrasil public protocol (`kernel.v1.outbound.execute` + `official/model-provider-lab`)
...
`packages/ydltavern-engine` passes PromptManager diagnostics, WI advanced diagnostics, nextState, and frames through `preset.compile` and `turn.generate`; 20 new deep-port JSON-RPC capabilities added. `model.live_call` / `model.live_call.stream` can make opt-in real provider HTTPS calls through `kernel.v1.outbound.execute` / `kernel.v1.outbound.stream` when the Yggdrasil host uses a live outbound profile; `model.live_realtime` can make OpenAI Realtime WebSocket calls through `kernel.v1.outbound.websocket.*` (Gemini Live is a best-effort stub). Generation actions without a real model path return explicit notices instead of appending fake assistant content.
