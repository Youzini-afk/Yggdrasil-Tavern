...
这仍是 `partial`。当前已有完整的 STScript runtime（scope chain、closure、pipe injection、abort/break、compareValues、registry + alias resolution）和完整的宏引擎。仍待完成：全部 153+ commands 实现、autocomplete/debugger、字节级 STScript 行为对齐。
