...
- R2-R5 已补齐 batches C/D/E/F；Round 7 X-track 又补齐 H-N。连同既有 A/B/G，当前 14 个 batches（A-N）约 150+ 个 slash command registrations 覆盖 199 个 ST canonical commands：真实实现、plan-only descriptor 与明确 unsupported sentinel 三类并存。
