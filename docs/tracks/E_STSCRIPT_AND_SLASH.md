...
- R2-R5 已补齐 batches C/D/E/F；连同既有 A/B/G，当前约 70 个 slash commands 有实现与测试覆盖。C 覆盖变量/控制，D 覆盖角色/群聊（危险删除为 unsupported，部分写入为 plan-only），E 覆盖消息可见性/编辑，F 覆盖 World Info injection 与相关 prompt entry 操作（部分委托 engine）。

这仍是 `partial`。当前已有完整的 STScript runtime（scope chain、closure、pipe injection、abort/break、compareValues、registry + alias resolution）、完整的宏引擎和约 70 个 batches A-G slash commands。仍待完成：剩余约 80 个 commands、把 plan-only / unsupported commands 推到可执行（如安全策略允许）、autocomplete/debugger、字节级 STScript 行为对齐。
