...
- generation lifecycle events are dispatched; when no real model path is wired, actions return explicit notices instead of appending fake assistant messages;
- unit tests cover push/edit/delete/splice, event dispatch, no-fake generation notices, the full macro engine, and STScript runtime.

This is still `partial`, not byte-level ST alignment. Next work is to expand more real DOM/network bridges, popups, third-party extension behavior, and provider payload alignment.
