...
`packages/ydltavern-engine` 的 `preset.compile` 和 `turn.generate` 会透传 PromptManager diagnostics、WI advanced diagnostics、nextState 和 frames；新增 20 个 deep-port JSON-RPC capability。`model.live_call` / `model.live_call.stream` 可在 Yggdrasil live outbound profile 下通过 `kernel.outbound.execute` / `kernel.outbound.stream` 发起 opt-in 真实 provider HTTPS；`model.live_realtime` 可通过 `kernel.outbound.websocket.*` 发起 OpenAI Realtime WebSocket（Gemini Live 为 best-effort stub）。默认 fake generation 仍不出网、不用 raw secret。
