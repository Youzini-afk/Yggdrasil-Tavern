...
| API Connections drawer | Provider/model/base URL/profile 选择 + API keys | paste + save 通过 Yggdrasil host RPC 把 raw key 保存到加密的 `official/secret-store-lab`，profiles 只保存 `secret_ref:store:NAME`；env `secret_ref:env:*` 仍可 fallback | implemented |
