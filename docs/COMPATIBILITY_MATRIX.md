...
这份文档是 YdlTavern 跟 SillyTavern 兼容覆盖率的对外雷达。它机械生成基线（ground truth 来自 ST 源码扫描），人工维护实现状态。
...
ST 核心运行时已完成一对一算法移植：PromptManager、World Info、STScript、宏引擎、chat/text completion 适配器、instruct mode、tokenizer registry、内置扩展逻辑、ST API surface、扩展加载器都有可运行代码路径，但除明确说明外不是全域字节级对齐。同窗口 ST 扩展兼容已落地：`messageFormatting()`（showdown + DOMPurify + hooks）、React DOM territory cession、`mountSTGlobals()`、ST URL layout shims 与 BME/shujuku 真实扩展 smoke。当前 golden diff 覆盖 20/20 scenarios：20 perfect、0 cosmetic、0 structural、0 unverifiable、0 error。

ST 源码仍是 ground truth；任何「全域字节级 ST 对齐」需要 fixture 与对齐测试支撑。
...
| 宏 / macros | 80+ | registry-based deep engine with full ST registry covering core/env/time/state/instruct/chat/variable + recursive expansion + PickState；golden harness 当前 4/4 perfect | implemented for current golden macro scenarios — `golden-harness/diff/macro-*.json` | `inventory/CORE_EVENTS_AND_COMMANDS.raw.md` | E |
| chat completion sources | 26 | 25 source request shapes ported with provider-specific overrides；golden harness 当前 4/4 perfect | implemented for current golden chat scenarios — `golden-harness/diff/chat-*.json` | `inventory/CONNECTORS_AND_SAMPLERS.raw.md` | C |
...
| 扩展 JS 执行 | ST extension JS | 同窗口 ST DOM fork：DOM anchors、`globalThis` bootstrap、ST ESM URL shims、full page APIs；QuickJS sandbox 仍可用于受限 synthetic tests | partial-real / same-window for real-extension smoke | `inventory/BUILTIN_EXTENSIONS.raw.md` | H |
| 真实模型调用 | provider HTTPS / WebSocket | `model.live_call` / `.stream` 通过 Yggdrasil `kernel.v1.outbound.execute` / `.stream` 发起 provider-final body，目前覆盖 OpenAI-compatible 与 Anthropic 路径；`model.live_realtime` bridge 到 `kernel.v1.outbound.websocket.*`（OpenAI Realtime real；Gemini Live best-effort stub）；API Connections 可通过 `official/secret-store-lab` 加密保存 key，profile 只保存安全 `secret_ref` | partial-opt-in | `inventory/CONNECTORS_AND_SAMPLERS.raw.md` | C |
...
| Benchmark coverage | 5 packages / 37 scenarios | `engine-core` 12、`importers` 6、`st-compat` 9、`extensions` 5、`surface` 5；根基线已提交到 `perf/baseline.json` | implemented baseline reference | `docs/guides/PERFORMANCE_BASELINE.md` | performance |
| Yggdrasil project install | Yggdrasil `project.yaml` + Home 项目卡 | 根目录 `project.yaml` 声明 `type: yggdrasil_native`、`entry_surface_id: ydltavern/play`、engine/surface 包和 required surfaces；`yg install` 后作为一等项目出现在 Home，支持 Play 启动 | implemented | `project.yaml` / `packages/ydltavern-surface/manifest.yaml` | platform |
...
| API Connections drawer | Provider/model/profile 选择 + API keys | paste + save/delete 通过 Yggdrasil host RPC 把 raw key 保存到加密的 `official/secret-store-lab`，profiles 只保存 `secret_ref:store:*`、`secret_ref:project:*` 或 `secret_ref:env:*`；任意 base URL 只作为 metadata，不会覆盖 live outbound host | implemented |
...
| Settings forms / drawers | ST drawer forms backed by shared state | Sampler / connection / formatting / persona / characters / world books / backgrounds / user settings 都读写 TavernProvider；schema-versioned localStorage 持久化 | implemented |
| Surface bundle | Browser-loadable ESM surface package | tsc emits `dist/index.js`；Vite emits browser-ready `dist/bundle.mjs`；CSS 与字体复制到 `dist/styles/` 与 `dist/fonts/` | implemented |
...
| `@ydltavern/st-compat` | D + E | live `chat[]` Proxy、Turn store、完整 `getContext()` shape（`context-st.ts`）、`eventSource`、`Generate`、宏 re-export（深实现位于 engine-core）、STScript 运行时（`stscript-st.ts`）、slash registry + batches A-N（~150 implemented/plan-only + ~40 unsupported，199/199 canonical coverage） | partial |
| `@ydltavern/engine-core` | C + I | chat/text request builders、stream chunk 状态机、token budget、PromptManager、World Info（token-approximation budget + seeded probability）、instruct mode、deep macro engine、tokenizer registry + `countTokens(text, options)` real adapters + HF runtime fetcher + guesstimate、golden harness fixtures、stream frames、model boundary plan | partial |
| `@ydltavern/surface` | G | Tavern-like product UI shell + `react-virtuoso` 虚拟聊天列表 + 9 wired drawers + TavernProvider settings/library state + browser-ready `dist/bundle.mjs` + 9 mount adapters + CSS/font asset copy + mobile responsive + diagnostic inspectors | implemented for current surface scope |
| `@ydltavern/extensions` | F + H | regex real engine、memory/vectors/quick-reply/token-counter executable logic、provider/IO-heavy extensions as plan/approximation、extension loader（manifest parse + validation + activation plan）、QuickJS sandbox runtime/bridge/ESM loader/permissions/audit/browser stubs；`realExtensionLoad` opt-in 支持 synthetic micro-BME 与 env-gated real BME smoke | partial-sandboxed / partial-opt-in |
| `@ydltavern/engine` | C | deep-port capabilities plus `model.live_call` / `model.live_call.stream` through Yggdrasil outbound execute/stream and `model.live_realtime` through outbound websocket；manifest declares provider hosts、WEBSOCKET methods 和 `secret_refs` | partial-opt-in |
...
## Slash command batch 覆盖

| Batch | 类别 | 覆盖 |
|---|---|---|
| A-G | 早期 batches（chat / messages / WI / persona / 等） | combined ~50+ implemented |
...
合计 ~150+ 个 slash command registrations + ~40 explicit unsupported sentinels = 199/199 ST canonical commands。Plan-only descriptors 返回 JSON `{ planned: true, action, fields }`；unsupported sentinels 抛出 `SlashCommandUnsupportedError`。
