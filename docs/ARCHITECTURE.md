...
Contract slice 已落地。仓库包含 shared types、资产导入器、ST 兼容运行时、引擎核心、Yggdrasil 子进程能力包和 `@ydltavern/surface` surface bundle；当前可以跑通 import / live `chat[]` edit / fake Generate / prompt request shape / surface event log 的最小纵切片。字节级 ST 对齐仍以兼容矩阵为准。
