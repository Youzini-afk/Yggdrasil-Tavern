...
YdlTavern 兼容 SillyTavern 的内容格式与扩展 API。本文是兼容范围的总图，具体兼容矩阵、扩展 API 表面、迁移路径会随实现展开持续填充。
...
实现方式：YdlTavern 直接读老格式，落到内部存储；同时调用 Yggdrasil 的通用能力包（`persona-lab` / `knowledge-lab` / `context-lab`）做 normalization。原始 payload 保留，不被破坏。
...
兼容层之外，引擎层走现代实现。这些能力来自 Yggdrasil，不是 YdlTavern 自己造的：
