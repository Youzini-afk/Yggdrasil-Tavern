...
下一步：补齐生产扩展 hosting（`/scripts/extensions/<id>/` host static route）、Activity Drawer 透明扩展审计、性能 baseline（子进程 JSON-RPC、SSE streaming、prompt construction），并推进 Phase B 痛点（multi-agent orchestration、MCP protocol surface、vector RAG、ToolManager full registration）。详细文档导航见 [`docs/`](docs/README.md)。
