...
- **兼容矩阵**：[`docs/COMPATIBILITY_MATRIX.md`](docs/COMPATIBILITY_MATRIX.md) —— B/C/D/E/F/G/H/I 进入 `partial`，深度移植覆盖 PromptManager / World Info / STScript / macros / chat/text completion / instruct / tokenizer / extensions / ST API / extension loader；Round 3 T-track 已验证 tokenizer 6/6 perfect、instruct ChatML/Llama3 2/2 perfect。
- **Golden harness**：[`golden-harness/`](golden-harness/) —— Node + jsdom 提取 harness，用只读 sibling SillyTavern 源码生成 chat / world-info / macro / instruct / tokenizer canonical fixtures；真实 `compare.mjs` diff workflow 已可运行，当前 20 个 scenarios 为 9 perfect、3 cosmetic、8 structural、0 unverifiable、0 error。详见 [`docs/guides/GOLDEN_HARNESS.md`](docs/guides/GOLDEN_HARNESS.md)。
- **YdlTavern 前端 surface**：[`packages/ydltavern-surface/`](packages/ydltavern-surface/) —— React surface bundle，`TavernPlaySurface` 现在默认展示 Tavern-like product UI：`react-virtuoso` 虚拟聊天列表、dark/light/parchment 主题系统、Connection/Sampler/Persona/Theme 分页设置、真实 loader-st 状态的 ExtensionsDrawer、QuickReplyBar 和移动端响应式布局；已提供 Yggdrasil-compliant `manifest.yaml`，并保留 `surface.manifest.json` 给 SurfaceHost 使用；仍不包含独立桌面/Web/App 壳。
...
下一步：继续闭合 chat / WI / macro 的 documented structural deltas，把更多 plan-only / unsupported slash 与扩展路径推进到可执行，并继续用字节级 fixtures 收紧 ST 对齐。详细文档导航见 [`docs/`](docs/README.md)。
