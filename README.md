...
- **兼容矩阵**：[`docs/COMPATIBILITY_MATRIX.md`](docs/COMPATIBILITY_MATRIX.md) —— B/C/D/E/F/G/H/I 进入 `partial`，深度移植覆盖 PromptManager / World Info / STScript / macros / chat/text completion / instruct / tokenizer / extensions / ST API / extension loader；tokenizer self-baseline 已由 R1 验证 6/6 perfect。
- **Golden harness**：[`golden-harness/`](golden-harness/) —— Node + jsdom 提取 harness，用只读 sibling SillyTavern 源码生成 chat / world-info / macro / instruct / tokenizer canonical fixtures；真实 `compare.mjs` diff workflow 已可运行，当前 20 个 scenarios 为 6 perfect（tokenizer self-baseline）、6 structural（chat/instruct）、8 unverifiable（WI/macro shim 限制）。详见 [`docs/guides/GOLDEN_HARNESS.md`](docs/guides/GOLDEN_HARNESS.md)。
...
下一步：扩大 golden harness shim 覆盖以提取真实 WI / macro fixtures，把更多 plan-only / unsupported slash 与扩展路径推进到可执行，并继续用字节级 fixtures 收紧 ST 对齐。详细文档导航见 [`docs/`](docs/README.md)。
