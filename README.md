...
- **兼容矩阵**：[`docs/COMPATIBILITY_MATRIX.md`](docs/COMPATIBILITY_MATRIX.md) —— B/C/D/E/F/G/H/I 进入 `partial`，深度移植覆盖 PromptManager / World Info / STScript / macros / chat/text completion / instruct / tokenizer / extensions / ST API / extension loader；Round 4 U-track 已验证 tokenizer 6/6、instruct 2/2、macro 4/4、world-info 4/4 byte-perfect，chat 4/4 cosmetic。
- **Golden harness**：[`golden-harness/`](golden-harness/) —— Node + jsdom 提取 harness，用只读 sibling SillyTavern 源码生成 chat / world-info / macro / instruct / tokenizer canonical fixtures；真实 `compare.mjs` diff workflow 已可运行，当前 20 个 scenarios 为 16 perfect、4 cosmetic、0 structural、0 unverifiable、0 error。详见 [`docs/guides/GOLDEN_HARNESS.md`](docs/guides/GOLDEN_HARNESS.md)。
...
下一步：把 chat 的 cosmetic-only 差异继续压到 byte-perfect，并扩展真实扩展加载的 ESM resolution、浏览器 stub、audited fetch bridge、Worker/WASM host bridge 与 BME 功能 smoke。详细文档导航见 [`docs/`](docs/README.md)。
