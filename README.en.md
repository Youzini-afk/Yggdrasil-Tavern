...
- **Compatibility matrix**: [`docs/COMPATIBILITY_MATRIX.md`](docs/COMPATIBILITY_MATRIX.en.md) — B/C/D/E/F/G/H/I are `partial`; deep-port covers PromptManager / World Info / STScript / macros / chat+text completion / instruct / tokenizer / extensions / ST API / extension loader; Round 4 U-track verifies tokenizer at 6/6, instruct at 2/2, macro at 4/4, and world-info at 4/4 byte-perfect, with chat at 4/4 cosmetic; Round 5 V-track lands SillyTavern UI parity (9/9 drawers, ST `.mes` message DOM, 1000px + 768px mobile breakpoints).
- **Golden harness**: [`golden-harness/`](golden-harness/) — Node + jsdom extraction harness that runs read-only sibling SillyTavern source and generates canonical chat / world-info / macro / instruct / tokenizer fixtures. The real `compare.mjs` diff workflow is operational: 20 scenarios currently produce 16 perfect, 4 cosmetic, 0 structural, 0 unverifiable, and 0 errors. See [`docs/guides/GOLDEN_HARNESS.md`](docs/guides/GOLDEN_HARNESS.en.md).
- **YdlTavern frontend surface**: [`packages/ydltavern-surface/`](packages/ydltavern-surface/) — React surface bundle; `TavernPlaySurface` now defaults to a SillyTavern parity product UI with 9 top-bar drawer surfaces, a 50vw `Sheld` main column, ST `.mes` message DOM structure, SendForm/StreamingIndicator/BackgroundLayer, ST theme JSON import/export, 3 YdlTavern native themes + 3 ST classic presets (Dark V 1.0 / Azure / Celestial Macaron), and 1000px + 768px mobile responsive breakpoints. `manifest.yaml` and `surface.manifest.json` now declare 9 surface contributions (the original 3 plus 6 specific drawer entries). It does not include an independent desktop/web/app shell. See [`docs/guides/UI_FORK_GUIDE.md`](docs/guides/UI_FORK_GUIDE.en.md).
...
Next: reduce the remaining chat cosmetic-only diffs to byte-perfect, and expand real extension loading with smarter ESM resolution, browser stubs, audited fetch bridging, Worker/WASM host bridges, and a richer BME functional smoke. Full documentation index in [`docs/`](docs/README.en.md).
