...
Next: complete production extension hosting (`/scripts/extensions/<id>/` host static route), add an Activity Drawer for transparent extension audit, establish performance baselines (subprocess JSON-RPC, SSE streaming, prompt construction), and advance Phase B pain points (multi-agent orchestration, MCP protocol surface, vector RAG, ToolManager full registration). Full documentation index in [`docs/`](docs/README.en.md).
