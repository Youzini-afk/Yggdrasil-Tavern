...
export function MessageComposer(): JSX.Element {
  ...
  return <section className="product-composer"><textarea value={tavern.input} onChange={(event) => tavern.setInput(event.target.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') void tavern.sendMessage(); }} placeholder="Type a message. Ctrl/⌘ + Enter sends." /><div className="composer-actions"><button type="button" onClick={() => { void tavern.sendMessage(); }}>Send</button><button type="button" onClick={tavern.generateReply}>Generate</button></div></section>;
}
