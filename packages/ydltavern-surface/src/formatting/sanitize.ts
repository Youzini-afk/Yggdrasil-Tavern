...
export function ensureDOMPurifyHooks(): void {
  ...
  DOMPurify.addHook('uponSanitizeAttribute', (_node, data, config) => {
    if (!(config as { MESSAGE_SANITIZE?: boolean } | undefined)?.MESSAGE_SANITIZE) return;
    ...
    if (data.attrName === 'class' && data.attrValue) {
      data.attrValue = data.attrValue
        ...
        .filter(Boolean)
        .map((cls) => {
          if (SAFE_CLASS_LITERALS.has(cls)) return cls;
          if (SAFE_CLASS_PREFIXES.test(cls)) return cls;
          return `custom-${cls}`;
        })
        .join(' ');
    }
...
export function sanitizeChatHtml(dirty: string, opts: SanitizeChatHtmlOptions = {}): string {
  ensureDOMPurifyHooks();
  const config: Config & { MESSAGE_SANITIZE: boolean; MESSAGE_ALLOW_SYSTEM_UI: boolean } = {
    RETURN_DOM: false,
