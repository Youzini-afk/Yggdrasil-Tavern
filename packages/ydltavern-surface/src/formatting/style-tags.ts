...
export function encodeStyleTags(text: string): string {
  return text.replace(STYLE_TAG_PATTERN, (_match, body: string) => {
    const encoded = encodeBase64(body);
    return `<custom-style data-encoded="${encoded}"></custom-style>`;
  });
}

export interface DecodeStyleOptions {
  prefix?: string;
}

export function decodeStyleTags(html: string, opts: DecodeStyleOptions = {}): string {
  ...
  return html.replace(
    ...
    (_match, encoded: string) => {
      ...
      const prefixed = prefix
        ? raw.replace(/(^|\})\s*([^\{\}]+)\s*\{/g, (_m, close: string, sel: string) => {
