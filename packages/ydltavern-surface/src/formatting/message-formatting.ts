...
import { _runPreMarkdown, _runPreSanitize, type FormatRenderCtx } from './hooks.js';
 ...
 */
export function formatMessage(raw: string, opts: FormatMessageOptions): string {
  if (!raw) return '';

  const ctx: FormatRenderCtx = {
  ...
  mes = mes.replace(/<code>([\s\S]*?)<\/code>/g, (_m, body: string) => {
  ...
  if (!opts.isSystem) {
    mes = mes.replace(new RegExp(`${QUOTE_SENTINEL}(\\d+)${QUOTE_SENTINEL}`, 'g'), (_m, idx: string) => {
