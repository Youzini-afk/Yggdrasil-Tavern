...
- `fetchHuggingFaceTokenizer(options)`：通过调用方提供的 Yggdrasil `kernelClient` 发起 `kernel.v1.outbound.execute`，下载 Hugging Face Hub 或指定 URL 的 `tokenizer.json`；支持 `expectedSha256` 完整性 pin、source=url、hf-hub model/revision 路径，以及内存 LRU cache。
