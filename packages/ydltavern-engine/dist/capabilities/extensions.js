import { analyzePromptChunks, applyRegexScripts, createExtensionHookRegistry, buildMemoryPromptInsertion, createExtensionRegistry, createSTExtensionCompatibilityAdapter, discoverExtensionBundles, parseExtensionManifest, normalizeMemorySettings, normalizeQuickReplySets, normalizeVectorSettings, planLoadExtension, planMemoryUpdate, planQuickReplyAutoExecute, planVectorIndex, planVectorInjection, planVectorQuery, sortExtensionsByLoadingOrder, } from "@ydltavern/extensions";
...
export const extensionHandlers = {
    [REGISTRY_ID]: (input) => {
        ...
        const registry = createExtensionRegistry(readArray(record["manifests"]));
        for (const id of readStringArray(record["disabled"] ?? record["disabledIds"])) {
            try {
                registry.setEnabled(id, false);
            }
            catch { /* diagnostic is enough at caller level */ }
        }
        return {
    ...
    [TOKEN_COUNTER_ID]: (input) => {
        const record = inputRecord(input);
        return {
            meta: createCapabilityMeta(TOKEN_COUNTER_ID, { deterministic: true }),
            ...analyzePromptChunks(readArray(record["chunks"])),
        };
    },
    [REGEX_ID]: (input) => {
        const record = inputRecord(input);
        return {
            ...
            ...applyRegexScripts(textField(input, "text"), readArray(record["scripts"]), {
                placement: readPlacement(record["placement"]),
                scopeId: typeof record["scopeId"] === "string" ? record["scopeId"] : undefined,
                enabledScopes: readOptionalStringArray(record["enabledScopes"]),
            }),
        };
    },
    [QUICK_REPLY_ID]: (input) => {
        const record = inputRecord(input);
        const normalized = normalizeQuickReplySets(readArray(record["sets"]));
        return {
...
function readHookKind(value) {
    return value === "slash" || value === "prompt" || value === "uiPanels" ? value : "events";
}
function readChat(value) {
    return value && typeof value === "object" && Array.isArray(value.turns)
        ? value
